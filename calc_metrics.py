...
def subprocess_fn(rank, args, temp_dir,generator,discriminator):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, 
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress,D=D)
        if rank == 0:
...
def calc_metrics(ctx, network_pkl,epoch, metrics, data,  gpus, verbose):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if args.num_gpus == 1:
            subprocess_fn(rank=0, args=args, temp_dir=temp_dir,generator=generator,discriminator=discriminator)
        else:
            torch.multiprocessing.spawn(fn=subprocess_fn, args=(args, temp_dir,generator,discriminator), nprocs=args.num_gpus)
