...
def train_vae():
    ...
    parser.add_argument('--network_pkl',  
                        default="training_runs/00001-SNGAN/checkpoints")
    ...
    experiment = VAEXperiment(model,
                            config['exp_params'], config['model_params']['latent_dim'],args.evaluate_interval)
...
def load_vae_model(args,config):
    if config['model_params']['name']=="VaeGan":
        ...
        if args.resume=="y":
            generator.load_state_dict(torch.load(args.network_pkl+"/gen_"+str(args.epoch)))
            discriminator.load_state_dict(torch.load(args.network_pkl+"/disc_"+str(args.epoch)),strict=False)
            generator.requires_grad_(False)
            if args.fine_tune_module=="e":
